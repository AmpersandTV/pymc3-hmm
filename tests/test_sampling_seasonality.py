...
from tests.utils import gen_defualt_params_seaonality, time_series, simulate_poiszero_hmm
...
def test_seasonality_sampling(N: int = 200, off_param=1):
    ...
    mean_error_rate = (1 - np.sum(np.equal(st_trace, simulation['S_t']) * 1) / len(simulation['S_t'])).values.tolist()
